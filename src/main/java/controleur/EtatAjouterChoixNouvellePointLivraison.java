...
public class EtatAjouterChoixNouvellePointLivraison extends EtatDefaut {
	...
	public void effectuerAjoutPointLivraison(long idDepart, long idNouvelle, int duree) throws Exception {
		...
		CommandeAjouterLivraison cmd = new CommandeAjouterLivraison(Controleur.getInstance().getCommandeIdPrece(),
				Controleur.getInstance().getCommandeId(), Controleur.getInstance().getCommandeVue(),
				Controleur.getInstance().getCommandeDuree());
